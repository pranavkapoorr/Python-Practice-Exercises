...
while True:
    ...
    number = numbers[math.ceil(len(numbers)/2)]
    guesses += 1
    response = input("is the number = "+ str(number) +" ?: ")
    if response == "yes" or response == "true":
        print("Yay I guessed it right in ",guesses," guesses")
        break
    elif response == "no" or response == "false":
        newResp = input("was the guess high or low with respect to actual number ?: ")
        if newResp == "high":
            numbers = numbers[0:numbers.index(number, )]
        elif newResp == "low":
